...
//constructor
    ...
    function addDP($modulo, $linea, $nroauditoria, $tipo_doc, $nro_doc, $codigo, $id_beneficio, $fecha) {
        if ($modulo == 'REL_PRACTICASSOLICITADASXAMBULATORIO') {  // No admite duplicados            
            // No admite el mismo codigo en la misma fecha            
            
              $query = "select nroauditoria from export_temp where modulo = 'REL_PRACTICASSOLICITADASXAMBULATORIO' and tipo_doc = '$tipo_doc' and nrodoc = '$nro_doc' and fecha='$fecha' and codigo = '$codigo'";
    ...
    function addDPM($modulo, $linea, $nroauditoria, $tipo_doc, $nro_doc, $codigo, $id_beneficio, $monto, $fecha) {

        if ($modulo == 'REL_PRACTICASREALIZADASXAMBULATORIO') {  // No admite duplicados
            // No admite el mismo codigo en la misma fecha
            $query = "select nroauditoria from export_temp where modulo = 'REL_PRACTICASREALIZADASXAMBULATORIO' and tipo_doc = '$tipo_doc' and nrodoc = '$nro_doc' and fecha='$fecha' and codigo = '$codigo'";
        ...
        $this->items = $this->items + 1;
        $query = "INSERT INTO export_temp (modulo, items, linea, nroauditoria,  tipo_doc, nrodoc, codigo, id_beneficio, monto, fecha) VALUES ('$modulo', $this->items, '$linea', $nroauditoria,  '$tipo_doc', '$nro_doc', '$codigo', '$id_beneficio', $monto, '$fecha')";
        $result = mysql_query($query);
        if (!$result)
            return false;
        else
    ...
    function getLineaUnica($modulo) {
        $query = "SELECT DISTINCT linea, id_beneficio FROM export_temp WHERE modulo = '$modulo'";        
        //$query = "SELECT DISTINCT linea, id_beneficio FROM export_temp WHERE modulo = '$modulo' ORDER by items";  // Falla en algunas versiones    
        $this->res = mysql_query($query);
        return $this->res;
    }
    ...
    function getPracticasPaciente($modulo, $tipo_doc, $nrodoc) {
        $query = "SELECT codigo, tipo_doc, nrodoc, linea, fecha FROM export_temp WHERE modulo = '$modulo' AND tipo_doc = '$tipo_doc' AND nrodoc = '$nrodoc' ORDER BY codigo";
        $this->res = mysql_query($query);
        return $this->res;
    }
    ...
    function getLineasPaciente($tipodoc, $nrodoc) {
        $query = "SELECT * FROM export_temp WHERE (modulo='AMBULATORIO' OR modulo='REL_DIAGNOSTICOSXAMBULATORIO' or modulo='REL_PRACTICASREALIZADASXAMBULATORIO' or modulo='REL_PRACTICASSOLICITADASXAMBULATORIO') AND TIPO_DOC='$tipodoc' AND NRODOC='$nrodoc' order by items";
        $this->res = mysql_query($query);
        return $this->res;
    }
