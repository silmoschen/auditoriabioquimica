...
while ($f = mysql_fetch_array($trans)) {
    $res = $exportar->getLineasPaciente($f['tipo_doc'], $f['nrodoc']);
    fwrite($archivo, 'PRESTACIONES' . " \r\n");
    $modulo_anter = '';
    while ($r = mysql_fetch_array($res)) {
        
        // Cierre para los casos donde hay mas de una orden en el mes
        if ($r['modulo'] == 'AMBULATORIO' && $modulo_anter != '') {
            fwrite($archivo, 'FIN AMBULATORIO' . " \r\n");
            fwrite($archivo, 'PRESTACIONES' . " \r\n");
        }
        
        if ($r['modulo'] != $modulo_anter) {
            fwrite($archivo, $r['modulo'] . " \r\n");
        }

        $fin_linea = '';
        if ($r['modulo'] == 'REL_PRACTICASREALIZADASXAMBULATORIO')
            $fin_linea = '1;1;';
        if ($r['modulo'] == 'REL_PRACTICASSOLICITADASXAMBULATORIO')
            $fin_linea = $r['cant'] + 1 . ';0;';

        fwrite($archivo, $r['linea'] . $fin_linea . " \r\n");
        $modulo_anter = $r['modulo'];
