...
$cosegurotx = new cTransaccionesCoseguroIapos();
...
while ($MostrarFila = mysql_fetch_array($Resultado)) {
    ...
    // Verificamos coseguro
    $msgcos = '';
    $cosegurotx->getUltimaTx($MostrarFila['nroauditoria']);
    if ($cosegurotx->nroauditoria != '') {
        if ($cosegurotx->ref2 == 0 && $cosegurotx->tarea == 'COSEGURO') {
            $msgcos = '<br/><font color = "purple">[' . $cosegurotx->mensaje . '] Tx. Iapos: ' . $cosegurotx->transiapos . ' Tx. B.: ' . $cosegurotx->ref1 .   '</font>';
            $ds = ' ';
        } else {
            $msgcos = '<br/><font color = "purple">' . $cosegurotx->ref3 . '</font>';            
            $ds = ' disabled = "false" ';
        }
    }    
    
    echo "<tr>";
    echo "<td " . $ext . " width='230px'><input type='checkbox' name='list' id='list' value=" . $MostrarFila['nroauditoria'] . $ds . "  onClick='chequear()' " . '>' . $MostrarFila['nroauditoria'] . ' ' . $estado . $msgcos . "</td>";
    ...
    if ($tc == 5) {
        ...
        echo "<td " . $ext . "  width='150px'>" . substr($afiliado->getNombre(), 0, 24) . $es . "</td>";
    } else {
