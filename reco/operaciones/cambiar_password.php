...
function ValidarPass(xpass){
	...
	ajax=objetoAjax();    
    ajax.open('GET', '/operaciones/validar_password.php?usuario='+usuario+'&pass='+xpass+'&aleatorio='+aleatorio);
...
function ValidarNuevoPass(e, xpass){
 ...
 if (intKey == 13) {
    if (xpass.length != 8) {
        alert('La Contraseña debe Contener 8 Caractéres ...!');
        return false;
    }  else {
        return true;
    }
...
function MP(xpass1, xpass2) {
   ...
   divError.innerHTML = '<i>Procesando</i>';
	...
	ajax=objetoAjax();
    ajax.open('GET', '/operaciones/modificar_password.php?usuario='+usuario+'&pass='+xpass1+'&aleatorio='+aleatorio);
	ajax.onreadystatechange=function() {
	  if (ajax.readyState==4) {
	    //mostrar resultados en esta capa
...
<td>
    <input name="passactual" id="passactual" type="password" style="font-size: 8px;" maxlength="8" width="2" size="10" onkeypress="javascript: if(ValidarPassActual(event, passactual.value)) {nuevopass.focus()}; return true" />
</td>
...
<td>
  <input name="nuevopass" id="nuevopass" type="password" disabled="true" style="font-size: 8px;" maxlength="8" width="2" size="10" onkeypress="javascript: if(ValidarNuevoPass(event, nuevopass.value)) {confirmapass.focus()}; return true" />
</td>
<td width="100px" align="left">8 caractéres</td>
</tr>

<tr>
<td width="110px" align="right">Confirmar Contraseña:</td>
<td>
    <input name="confirmapass" id="confirmapass" type="password" disabled="true" style="font-size: 8px;" maxlength="8" width="2" size="10" onkeypress="javascript: if(ValidarConfirmacion(event, confirmapass.value, nuevopass.value)) {registrar.focus()}; return true" />
</td>
