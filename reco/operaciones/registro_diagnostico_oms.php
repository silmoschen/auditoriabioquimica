...
if (strlen(trim($oms)) == 0) { $oms = 'N';}

if (strlen(trim($oms_cod)) > 0 and strlen(trim($clave)) > 0 and strlen(trim($descrip)) > 0 and $l) {
  //creamos el objeto $objempleados
  //y usamos su m�todo crear
  sleep(2);
  $obj=new cDiagnosticosOMS;
  if ($obj->crear($oms_cod, $clave, $orden, $indice, $codrap, $descrip, $oms)==true){	           
  }else{
